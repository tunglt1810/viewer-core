...
class Node {
    ...
    clearPath(path, recursively) {
        ...
        if (path !== null) {
            const last = path.length - 1;
            const node = this.getNodeUpToIndex(path, last);
            if (node !== null) {
                let item = path[last];
                if (item !== WILDCARD) {
                    if (node.children.hasOwnProperty(item)) {
    ...
    attachHandlerForPath(path, type, handler) {
        ...
        if (path !== null) {
            const node = this.getNodeUpToIndex(path, path.length);
            if (node !== null) {
...
export const unsavedChanges = {
   ...
   */
    attachHandler(path, type, handler) {
        return (
   ...
   * @return {Boolean} Returns true on success and false on failure.
   */
    removeHandler(path, type, handler) {
   ...
   */
    trigger(path, type, nonRecursively) {
   ...
   * @return {void} No value is returned.
   */
    checkBeforeAction(path, callback, options) {
        let probe; let hasChanges; let
            shouldProceed;
   ...
   * @return {void} No value is returned.
   */
    presentProactiveDialog(path, callback, options) {
        let probe; let
            hasChanges;
        ...
        if (probe > 0) {
            // Unsaved changes exist...
            hasChanges = true;
            OHIF.ui.unsavedChangesDialog((choice) => {
