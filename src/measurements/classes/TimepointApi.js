...
export default class TimepointApi {
  ...
  calculateVisitNumber(timepoint) {
      ...
      const sortedTimepoints = this.timepoints.sort((tp1, tp2) => (tp1.visitDate > tp2.visitDate ? 1 : -1));
      const filteredTimepoints = sortedTimepoints.find(
          (tp) => tp.PatientID === timepoint.PatientID &&
  ...
  all(filter) {
      ...
      return timepointsToReturn.sort((tp1, tp2) => (tp1.visitDate < tp2.visitDate ? 1 : -1));
  }
  ...
  initialTimepointIds() {
      ...
      this.timepoints.filter(
          (tp) => tp.timepointType === 'prebaseline' && tp.visitDate <= visitDateToCheck
      ) || [];
      ...
      this.timepoints.filter(
          (tp) => tp.timepointType === 'baseline' && tp.visitDate <= visitDateToCheck
      ) || [];
  ...
  nadir() {
      ...
      const nadir = this.all().find(
          (tp) => tp.timepointId !== current.timepointId &&
